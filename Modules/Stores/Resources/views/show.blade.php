...
<div class="app-content">
      ...
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{route('Stores.update',['id' => encode($store->id)])}}">
                {{csrf_field()}}
            ...
            <div class="tile-body ">
              <div class="col-md-6">
                ...
                <div class="form-group">
                  ...
                  <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" type="text" name="name" value="{{$store->name}}" placeholder="Enter full name">
                   @if ($errors->has('name'))
             <div class="invalid-feedback">{{ $errors->first('name') }}</div>
                                @endif
                </div>
                <div class="form-group">
                  <label class="control-label">@lang('stores::global.BaseUrl') <strong>*</strong></label>
                  <input class="form-control{{ $errors->has('base_url') ? ' is-invalid' : '' }}" type="text" name="base_url" value="{{$store->base_url}}" placeholder="Enter Base Url">
                   @if ($errors->has('base_url'))
             <div class="invalid-feedback">{{ $errors->first('base_url') }}</div>
                                @endif
                </div>
                <div class="form-group">
                  <label class="control-label">@lang('stores::global.ApiUrl') <strong>*</strong></label>
                  <input class="form-control{{ $errors->has('api_url') ? ' is-invalid' : '' }}" type="text" name="api_url" value="{{$store->api_url}}" placeholder="Enter Api Url">
                   @if ($errors->has('api_url'))
             <div class="invalid-feedback">{{ $errors->first('api_url') }}</div>
                                @endif
                </div>
                 <div class="form-group">
                  <label class="control-label">@lang('stores::global.ApiLogin') <strong>*</strong></label>
                  <input class="form-control{{ $errors->has('api_login') ? ' is-invalid' : '' }}" type="text" name="api_login" value="{{$store->api_login}}" placeholder="Enter Api login">
             ...
             <div class="invalid-feedback">{{ $errors->first('api_login') }}</div>
                 ...
                 <div class="form-group">
                  ...
                  <input class="form-control{{ $errors->has('api_password') ? ' is-invalid' : '' }}" type="password" name="api_password" value="" placeholder="Enter Api Password">
