            ...
            <div class="tile-body ">
             ...
             <div class="invalid-feedback">{{ $errors->first('quantity') }}</div>
                 ...
                 <div class="form-group">
                  ...
                  <select multiple value="{{collect($product->extension_attributes->category_links)->pluck('category_id')}}" name="category" class="form-control">
                     @foreach($categories as $c)

                     <option @if(collect($product->extension_attributes->category_links)->contains('category_id',$c->id))selected @endif value="{{$c->id}}">
                      {{$c->name}}
                     </option>

                     @if(count($c->children_data))
                     @foreach($c->children_data as $cd)
                     <option @if(collect($product->extension_attributes->category_links)->contains('category_id',$cd->id))selected @endif  value="{{$cd->id}}">{{$cd->name}}
                     </option>
                      @if(count($cd->children_data))
                     @foreach($cd->children_data as $cdd)
                      <option @if(collect($product->extension_attributes->category_links)->contains('category_id',$cdd->id))selected @endif  value="{{$cdd->id}}">{{$cdd->name}}</option>
                  ...
                  <input class="form-control{{ $errors->has('category') ? ' is-invalid' : '' }}" type="password" name="category" value="" >
