                        ...
                        <table class="table table-hover table-bordered" id="storesTable">
                            ...
                            @foreach($stores as $store)
                                <tr>
                                    ...
                                    <td><a href="{{route('Stores.show',['id' => encode($store->id)])}}">
                                            <button class="btn btn-sm btn-primary">{{trans('global.Edit')}}</button>
                                        </a>
                                        <form class="d-inline" method="post"
                                              action="{{route('Stores.delete',['id' => encode($store->id)])}}">
                                            {{csrf_field()}}
                                            <input type="hidden" name="_method" value="DELETE">

                                            <button type="submit"
                                                    class="btn btn-sm btn-danger deleteStoreBtn">{{trans('global.Delete')}}</button>
                                        </form>
                                        <a href="{{route('Store.index',['id' => encode($store->id)])}}">
                                            <button class="btn btn-sm btn-primary">{{trans('global.Manage')}}</button>
                                        </a>
                                    </td>
